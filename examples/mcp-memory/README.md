...
These three **default-off reference configurations** connect one memory system to DSH through [`@xneog/dsh-mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.
...
```sh
xneog web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
xneog web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
xneog web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
