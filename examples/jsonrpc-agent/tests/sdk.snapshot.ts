 ...
 * REAL `@xneog/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@xneog/dsh-acp-snapshot'
import { resolveExampleLaunch } from '@xneog/dsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@xneog/dsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@xneog/dsh-system-prompt') return []
