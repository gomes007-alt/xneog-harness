...
function clientConfigs(id = '@xneog/dsh-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@xneog/dsh-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@xneog/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@xneog/dsh-client-web-react')).toBeNull()
    expect(resolveId('@xneog/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@xneog/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@xneog/dsh-host-apiproxy/api')).toBeNull()
    expect(resolveId('@xneog/dsh-session/surface')).toBeNull()
    expect(resolveId('@xneog/dsh-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@xneog/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@xneog/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@xneog/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@xneog/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @xneog leak', () => {
    expect(() => resolveId('@xneog/dsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@xneog/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@xneog/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@xneog/dsh-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@xneog/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@xneog/dsh-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@xneog/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@xneog/dsh-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@xneog/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@xneog/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@xneog/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@xneog/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@xneog/dsh-client-connection/client.js.map')
