...
const SESSION_PACKAGE = '@xneog/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@xneog/dsh-session/types'
 ...
 * top-level declaration (in `@xneog/dsh-session`) and any declaration
 * merge inside a `declare module '@xneog/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @xneog/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
