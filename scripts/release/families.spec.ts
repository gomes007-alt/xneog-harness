...
describe('release families', () => {
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@xneog/dsh')
    const cordis = { ...member('vendor/cordis', '@xneog/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@xneog/dsh'), { ...member('apps/web', '@xneog/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@xneog/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@xneog/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@xneog/dsh-consumer', { dependencies: { '@xneog/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@xneog/dsh-library'),
      member('packages/a/zebra', '@xneog/dsh-zebra'),
    ]
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@xneog/dsh-left', { dependencies: { '@xneog/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@xneog/dsh-right', { dependencies: { '@xneog/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@xneog/dsh-consumer', { peerDependencies: { '@xneog/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@xneog/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@xneog/dsh-left', { peerDependencies: { '@xneog/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@xneog/dsh-right', { peerDependencies: { '@xneog/dsh-left': 'workspace:^' } }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@xneog/dsh-right', peer: '@xneog/dsh-left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@xneog/dsh-base', { peerDependencies: { '@xneog/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@xneog/dsh-consumer', {
        dependencies: { '@xneog/dsh-base': 'workspace:^' },
        peerDependencies: { '@xneog/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@xneog/dsh-base', peer: '@xneog/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@xneog/dsh-alpha', { peerDependencies: { '@xneog/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@xneog/dsh-bravo', { peerDependencies: { '@xneog/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@xneog/dsh-charlie', { dependencies: { '@xneog/dsh-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @xneog\/dsh-charlie -> @xneog\/dsh-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@xneog/dsh-alpha', { devDependencies: { '@xneog/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@xneog/dsh-zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@xneog/dsh-library')
    const vendored = member('vendor/cordis', '@xneog/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@xneog/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@xneog/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@xneog/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@xneog/dsh-library', { files: ['lib/index.js'] }),
