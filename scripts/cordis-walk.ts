...
const MERGE_HEAD = /declare module ['"](?:@xneog\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@xneog/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@xneog/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@xneog/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@xneog/cordis'` Context merge
...
/**
 * Every event name a `declare module '@xneog/cordis'` Events merge declares in one
