...
const vendoredPackages = new Set([
  ...
  '@xneog/cosmokit',
  '@xneog/schemastery',
  '@xneog/cordis-plugin-loader',
  '@xneog/cordis-plugin-include',
  '@xneog/cordis-plugin-group',
  '@xneog/cordis-plugin-timer',
  '@xneog/cordis-plugin-hmr',
  '@xneog/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@xneog/node-addon-landlock-run',
  '@xneog/node-addon-landlock-run-linux-arm64',
  '@xneog/node-addon-landlock-run-linux-x64',
])
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@xneog/dsh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@xneog/dsh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@xneog/dsh-base': ['cordis.patch.yml'],
  '@xneog/dsh-web-app': ['cordis.patch.yml'],
  '@xneog/dsh-headless': ['cordis.patch.yml'],
  '@xneog/dsh-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@xneog/dsh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@xneog/dsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@xneog/dsh-skill-badge': ['assets'],
  '@xneog/dsh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@xneog/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@xneog/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@xneog/dsh-')) {
    const peer = manifest.peerDependencies?.['@xneog/cordis']
