...
const CHOOSER_PACKAGE = '@xneog/dsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@xneog/dsh-host-directory-picker-native',
  '@xneog/dsh-host-directory-picker-browse',
  '@xneog/dsh-client-ui-directory-picker-browse',
  '@xneog/dsh-client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@xneog/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@xneog/cordis-plugin-include') return
