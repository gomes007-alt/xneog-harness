...
function checkManifest(
  ...
  if (owner.packageName === '@xneog/dsh-invariants') return
  if (manifest.peerDependencies?.['@xneog/dsh-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@xneog/dsh-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@xneog/dsh-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@xneog/dsh-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@xneog/dsh-invariants'
