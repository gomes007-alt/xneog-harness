...
const RELEASE_ENTRY_PACKAGE = '@xneog/dsh'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"xneog-harness web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed xneog web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed xneog web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@xneog/')) {
        throw new Error(`${manifestPath} must name an @xneog package`)
      }
      if (name === '@xneog/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@xneog/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@xneog/') || name === '@xneog/dsh-root')) {
