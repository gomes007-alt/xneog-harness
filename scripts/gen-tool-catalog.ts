...
import type { ToolSchema } from '@xneog/dsh-llm'
import AgentRegistry from '@xneog/dsh-agent'
import type { Agent } from '@xneog/dsh-agent'
import { createScope } from '@xneog/dsh-scope'
import SessionStore, { SessionId } from '@xneog/dsh-session'
import SessionProjectionRegistry from '@xneog/dsh-session-projection'
import SqliteSessionQueryEngine from '@xneog/dsh-session-query-sqlite'
import GoalService from '@xneog/dsh-goal'
import SystemPrompt from '@xneog/dsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@xneog/dsh-tools'
import LocalBashExecutor from '@xneog/dsh-bash-local'
import * as BashEnvPlugin from '@xneog/dsh-shell-env'
import { PwshLocalExecutor } from '@xneog/dsh-pwsh-local'
import LocalSubprocessRuntime from '@xneog/dsh-subprocess-local'
import LocalFileSystem from '@xneog/dsh-fs-local'
import { AttachmentStore } from '@xneog/dsh-attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@xneog/dsh-attachment'
import UserQuestionService from '@xneog/dsh-user-questions'
import PlanModeController from '@xneog/dsh-plan-mode'
import WebRuntime from '@xneog/dsh-web'
import * as WebSearchExa from '@xneog/dsh-web-search-exa'
import * as WebFetchLocal from '@xneog/dsh-web-fetch-http'
import SubagentRuntime from '@xneog/dsh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@xneog/dsh-subagent'
import * as ToolSubagentControl from '@xneog/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@xneog/dsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@xneog/dsh-tool-subagent-report'
import SkillRegistry from '@xneog/dsh-skill'
import * as SkillFileSystem from '@xneog/dsh-skill-filesystem'
import LocalJobRegistry from '@xneog/dsh-jobs-local'
import * as ToolAskUser from '@xneog/dsh-tool-ask-user'
import * as ToolBash from '@xneog/dsh-tool-bash'
import * as ToolPwsh from '@xneog/dsh-tool-pwsh'
import * as ToolBashPersistent from '@xneog/dsh-tool-bash-persistent'
import CordisHostRunner from '@xneog/dsh-cordis-host-runner'
import * as ToolCordis from '@xneog/dsh-tool-cordis'
import * as ToolFs from '@xneog/dsh-tool-fs'
import * as ToolFsSearch from '@xneog/dsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@xneog/dsh-tool-str-replace-editor'
import TerminalSessionService from '@xneog/dsh-terminal'
import * as ToolPty from '@xneog/dsh-tool-terminal'
import * as ToolGoal from '@xneog/dsh-tool-goal'
import * as ToolSchedule from '@xneog/dsh-schedule'
import Lsp from '@xneog/dsh-lsp'
import * as ToolLsp from '@xneog/dsh-tool-lsp'
import * as ToolSkill from '@xneog/dsh-tool-skill'
import * as ToolSessionQuery from '@xneog/dsh-tool-session-query'
import * as ToolTasks from '@xneog/dsh-tool-jobs'
import * as ToolTodo from '@xneog/dsh-tool-todo'
import * as ToolSubagent from '@xneog/dsh-tool-subagent'
import * as ToolWeb from '@xneog/dsh-tool-web'
import VmWorkflowEngine from '@xneog/dsh-workflow-worker-thread'
import * as ToolRalph from '@xneog/dsh-tool-ralph'
import * as ToolWorkflow from '@xneog/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@xneog/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@xneog/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@xneog/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@xneog/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@xneog/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@xneog/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@xneog/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@xneog/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@xneog/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@xneog/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@xneog/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@xneog/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@xneog/dsh-tool-session-query',
  ...
  {
    pkg: '@xneog/dsh-tool-subagent-control',
  ...
  {
    pkg: '@xneog/dsh-tool-subagent-report',
