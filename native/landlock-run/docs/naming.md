...
The public package family belongs to the `@xneog` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@xneog/node-addon-landlock-run
@xneog/node-addon-landlock-run-<platform>
```
