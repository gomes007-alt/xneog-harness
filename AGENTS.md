...
xneog is a plugin-based agent harness on vendored Cordis: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.
...
packages/    @xneog/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  bundle/      installable xneog --profile patch-layer bundles
...
native/      @xneog/node-addon-landlock-run source of record (see native/README.md)
...
pnpm xneog --profile headless "task"  # run one task from source (needs DEEPSEEK_API_KEY)
...
- Every npm package is `@xneog/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@xneog/cordis` is a peerDependency (+ dev) of every harness package.
