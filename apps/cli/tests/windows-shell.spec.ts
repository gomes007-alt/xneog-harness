...
import { entryListSchema } from '@xneog/cordis-plugin-include'
import { evaluate } from '@xneog/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@xneog/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@xneog/dsh-base', '@xneog/dsh-web-app'])
    ...
    for (const name of ['@xneog/dsh-pwsh-sandbox', '@xneog/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@xneog/dsh-base'])
