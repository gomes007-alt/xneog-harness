...
import { startMockLlmServer } from '@xneog/dsh-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@xneog/dsh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@xneog/dsh-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@xneog/dsh-cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('xneog --profile web')
    expect(help.stdout).toContain('xneog plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(web.stdout).toContain('Usage: xneog --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('xneog web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('xneog web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: xneog --profile headless')
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('xneog plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `xneog plugin --profile x add .` from a plugin checkout must install THAT
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@xneog/dsh-base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@xneog/dsh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@xneog/dsh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @xneog/dsh-base')
      expect(stdout).toContain("name: '@xneog/dsh-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@xneog/dsh-headless'")
      expect(stdout).not.toMatch(/name: '@xneog\/dsh-host-/)
      expect(stdout).not.toContain("name: '@xneog/dsh-web-app'")
      expect(stdout).not.toMatch(/name: '@xneog\/dsh-client-/)
    }, 30_000)
