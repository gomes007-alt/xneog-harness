...
import { DSH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@xneog/dsh-launch-environment'
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@xneog/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@xneog/cordis-plugin-hmr', config: { root: [] } })
      }
