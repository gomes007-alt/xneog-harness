...
the deliberate composition divergences from `xneog web` — are documented in
...
## Do not import `@xneog/dsh-client-*` here
...
`@xneog/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@xneog/dsh-client-web` and the boot-manifest
type from `@xneog/dsh-client-modules/client`: booting the real shell is what
...
`@xneog/dsh-client-runtime/client` because `client/runtime` is reachable
