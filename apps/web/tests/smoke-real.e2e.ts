// Real-host smoke: spawn `xneog web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`xneog web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /xneog web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`xneog web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `xneog web` serves every production manifest plugin; until every UI
...
describe('xneog web keyless CLI smoke', () => {
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () => document.title !== 'xneog' && document.title.endsWith(' — xneog'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — xneog`,
      durableTitle,
      { timeout: 15_000 },
    )
