...
`dsh --dump-config` and `xneog web --dump-config` print the composed entry list — base, surface overlay, then the `--config` or personal overlay, exactly the layers that surface's boot assembles — as YAML on stdout and exit without booting. `dsh --dump-default-config` / `xneog web --dump-default-config` stop at the surface overlay, so diffing the two outputs shows precisely what the user layer changes.
