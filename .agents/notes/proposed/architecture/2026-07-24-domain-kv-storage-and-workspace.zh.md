...
| 包 | 路径 | ctx 面 | 本期 |
| --- | --- | --- | --- |
| `@xneog/dsh-storage` | `packages/storage/storage/` | `ctx.storage`（枢纽） | ✓ |
| `@xneog/dsh-storage-json` | `packages/storage/storage-json/` | 注册后端 `json` | ✓ |
| `@xneog/dsh-storage-sqlite` | `packages/storage/storage-sqlite/` | 注册后端 `sqlite` | ✓ |
| `@xneog/dsh-storage-domain` | `packages/storage/storage-domain/` | 挂载 `ctx.storage.domain` | ✓ |
