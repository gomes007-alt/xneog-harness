...
[Upstream's Issue lifecycle workflow](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-lifecycle.yml) remains unsubscribed from `pull_request.ready_for_review`; neither event command depends on that action. [Upstream's Issue policy workflow](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-policy.yml) retains `ready_for_review` because it owns required-check enforcement when a human pull request enters review. Neither workflow was ported to this fork, so the policy currently runs only through its shipped tests.
