...
[上游的 Issue 生命周期工作流](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-lifecycle.yml)仍不订阅 `pull_request.ready_for_review`；两条事件命令均不依赖该动作。[上游的 Issue 策略工作流](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-policy.yml)保留 `ready_for_review`，因为人工提交的 PR 进入评审时，该工作流负责执行必需检查门禁。两个工作流均未移植到本 fork，因此该策略目前只通过随仓库发布的测试运行。
