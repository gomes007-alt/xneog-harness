...
The shipped bundles are `@xneog/dsh-base` (shared core rows), `@xneog/dsh-web-app` (browser Host rows and Web runtime glue), and `@xneog/dsh-headless` (a direct one-shot runner over base, without web-app). Generic `xneog --profile <name>` hands its remaining arguments to that profile's command-line startup row: Web owns its flag family, while headless owns its task positional. Patch overlays use launcher-owned `--patch`. `xneog plugin --profile <name> <args...>` is a thin pnpm forwarder that initializes the profile and reconciles `dsh.profile.bundles` with installed bundle declarations; a package without a bundle declaration remains a plain dependency. [Headless as a direct core entry point](2026-08-09-headless-direct-core-entry-point.md) owns the headless composition contract.
