...
- A deployment that supervises `xneog --profile headless` with SIGTERM now observes exit 0 instead of 143; the caller sent the signal and sees no answer on stdout.
- The reload chain runs in every `xneog web` process; a deployment that must not expose `/plugins/events` disables the `client-hmr` row in its patch layer.
