...
1. `@xneog/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@xneog/dsh-web-search-exa`, `@xneog/dsh-web-search-perplexity`, `@xneog/dsh-web-search-deepseek`, and `@xneog/dsh-web-fetch-http`.
3. `@xneog/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@xneog/dsh-tool-web  --depends on-->  @xneog/dsh-web  <--depends on--  @xneog/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @xneog/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @xneog/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @xneog/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@xneog/dsh-web-search-exa"] -->|registerSearchProvider| web["@xneog/dsh-web / ctx.web"]
  perplexity["@xneog/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@xneog/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@xneog/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@xneog/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@xneog/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@xneog/dsh-tool-web` depends on `@xneog/dsh-web`, `@xneog/dsh-tools`, `@xneog/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@xneog/dsh-web'
...
- id: web-search-perplexity
  name: '@xneog/dsh-web-search-perplexity'
