...
- One `dsh` process supporting both `xneog web` (serve) and `xneog --profile headless` (headless) — one process, two modes (a design reservation)
- Launching inside Electron with the same Web technologies as `xneog web`
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/cli` (`@xneog/dsh`) dispatches commands: `xneog web` = Host + webserver + the built `dsh-web-frontend` dist; `xneog --profile headless` = [a direct core Agent/Session entry point](2026-08-09-headless-direct-core-entry-point.md), with zero Host, HTTP, or browser layer.
...
The two existing applications preserve the division: the Web application mounts Host, carrier, and browser composition, while `xneog --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
|---|---|---|---|
| `InProcessApiClient` | apiproxy itself | the injected `{ fetch }` handler | **The isomorphic point**: `new InProcessApiClient(toFetchHandler(api))` never touches the network yet runs the real wire serialization/zod/SSE framing; carrier tests and callers can exercise the protocol without opening a port, while product `xneog --profile headless` drives core directly |
