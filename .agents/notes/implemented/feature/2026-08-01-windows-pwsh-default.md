...
Windows hosts booting a shipped profile (`xneog web`, `xneog --profile headless`, one-shot tasks) get the PowerShell stack by default; POSIX hosts are unchanged.
...
- Keyless: a `xneog --profile <name> --dump-config` shows both stacks in the one shared patch layer, with each row's own `disabled` expression deciding the roster at mount.
