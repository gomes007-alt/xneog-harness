...
Every `pnpm/action-setup` step in [upstream's primary CI workflow](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/ci.yml) — not ported to this fork — sets `dest: ${{ runner.temp }}/setup-pnpm`. Each runner service owns its temporary directory, so one setup cannot replace another runner's install directory. Persistent store reuse remains separate through `PNPM_CONFIG_STORE_DIR`, as established by the [pnpm provisioning decision](../process/2026-07-26-pnpm-action-setup-for-symmetric-ci-caching.md).

[The workflow regression test](../../../../scripts/ci-workflow.spec.ts) discovers every `pnpm/action-setup` step in `ci.yml` and rejects one without the runner-private destination; the spec ships in this fork and fails until the workflow is restored. This keeps newly added jobs inside the same isolation boundary.
