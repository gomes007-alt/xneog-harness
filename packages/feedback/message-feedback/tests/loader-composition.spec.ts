...
import * as StorageDomain from '@xneog/dsh-storage-domain'
import * as StorageJson from '@xneog/dsh-storage-json'
import { remoteMethods } from '@xneog/dsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@xneog/dsh-session', SessionStore],
    ['@xneog/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@xneog/dsh-storage', Storage],
    ['@xneog/dsh-storage-json', StorageJson],
    ['@xneog/dsh-storage-domain', StorageDomain],
    ['@xneog/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@xneog/dsh-session-persistence-jsonl'",
      '  config:',
      ...
      '  config:',
      '    backend: json',
      "- name: '@xneog/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
