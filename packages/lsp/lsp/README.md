...
| Package | Role |
|---|---|
| `@xneog/dsh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@xneog/dsh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@xneog/dsh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
