...
/**
 * Keyless built-artifact smoke: plain Node imports `@xneog/dsh-lsp` and
 * `@xneog/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@xneog/cordis')
      const { default: Lsp } = await import('@xneog/dsh-lsp')
      const LspLocal = await import('@xneog/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@xneog/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@xneog/dsh-subprocess-local')
