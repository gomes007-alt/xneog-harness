...
```
@xneog/cordis-plugin-timer  timer service (writes nothing to stdout)
@xneog/dsh-llm              abstract LLM service + content-block vocabulary
@xneog/dsh-session          event-sourced session log + store
@xneog/dsh-session-title    log-backed title service + deterministic fallback
@xneog/dsh-system-prompt    prompt-section + tool-schema assembly
@xneog/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@xneog/dsh-skill            skill provider registry
@xneog/dsh-skill-filesystem      local filesystem skill provider
@xneog/dsh-agent            agent registry + initiator scope + agent/* events
@xneog/dsh-goal             optional persisted same-session goal domain
@xneog/dsh-tool-goal        optional model-facing goal controls
@xneog/dsh-goal-round-driver     optional same-session goal-round driver
@xneog/dsh-llm-retry        provider-routed request retry policy
@xneog/dsh-jobs-local      generic background-job registry
@xneog/dsh-invariants       configurable invariant registry service
@xneog/dsh-session/invariant
@xneog/dsh-agent/invariant
@xneog/dsh-scope/invariant
@xneog/dsh-agent-loop/invariant
                                  package-owned relational checks
@xneog/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@xneog/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@xneog/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@xneog/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@xneog/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@xneog/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@xneog/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
