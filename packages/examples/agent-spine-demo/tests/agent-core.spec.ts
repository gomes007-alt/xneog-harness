...
import { renderPrompt, TOOL_ORDER_REST } from '@xneog/dsh-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@xneog/dsh-agent'
...
import type { ToolExecution } from '@xneog/dsh-tools'
...
/**
 * Unit coverage for the @xneog/dsh-agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@xneog/dsh-agent$'] },
      { package_blocklist: ['^@xneog/dsh-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by xneog.')
