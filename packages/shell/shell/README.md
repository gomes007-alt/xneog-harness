...
| Package | Role |
|---|---|
| `@xneog/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@xneog/dsh-bash-local` | Service Provider: local subprocesses |
| `@xneog/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@xneog/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
