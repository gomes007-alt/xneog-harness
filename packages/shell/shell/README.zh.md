...
| 包 | 职责 |
|---|---|
| `@xneog/dsh-shell`（本包） | Service Definition：抽象服务 + 词汇类型 |
| `@xneog/dsh-bash-local` | Service Provider：本地子进程 |
| `@xneog/dsh-bash-sandbox` | Service Provider：沿用 `dsh-bash-local` 的机制，但通过 [`ctx.sandbox`](../../sandbox/sandbox/) 限制每次 spawn，并将拒绝报告为结果事实 |
| `@xneog/dsh-tool-bash` | 基于 `ctx.shell`、面向模型的工具 schema |
