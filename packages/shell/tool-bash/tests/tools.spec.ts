...
import { ShellExecutor } from '@xneog/dsh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@xneog/dsh-shell'
import SystemPrompt from '@xneog/dsh-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@xneog/dsh-tools'
...
import ApprovalService from '@xneog/dsh-user-approval'
import type { ApprovalOutcome } from '@xneog/dsh-user-approval'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @xneog/dsh-jobs and @xneog/dsh-tool-jobs')
  })
