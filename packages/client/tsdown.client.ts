 ...
 * Everything else under @xneog/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@xneog\/dsh-(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@xneog\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@xneog/dsh-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @xneog value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@xneog/')) return null
