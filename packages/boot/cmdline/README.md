...
- `ctx.cmdlineArgs` — the invocation's inner arguments. `get()` is the whole interface, and it returns a snapshot: `xneog --profile tui --resume abc` yields `['--resume', 'abc']`.
...
```yaml
- id: web-startup
  name: '@xneog/dsh-web-app/startup'
```
