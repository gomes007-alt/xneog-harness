 ...
 * optional user patch layers from the Harness home (`~/.xneog-harness`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @xneog/dsh-app-boot
 */
...
import { Context, type FiberState } from '@xneog/cordis'
import Loader, { type Entry, type EntryOptions } from '@xneog/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@xneog/cordis-plugin-include'
import Group from '@xneog/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@xneog/dsh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@xneog/dsh-launch-environment'
import type {} from '@xneog/cordis-plugin-hmr'
// Side-effect type import: resolves `ctx.get('systemPrompt')` to the service.
import type {} from '@xneog/dsh-system-prompt'
 ...
 * entries (`@xneog/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@xneog/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@xneog/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The xneog implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend xneog itself.`,
  })
}
