 ...
 * @module @xneog/dsh-app-boot/profile
 */
...
import type { EntryOptions } from '@xneog/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@xneog/cordis-plugin-include'
 ...
 * @param name - the profile name (`xneog --profile <name>`).
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@xneog/dsh-base', '@xneog/dsh-web-app'],
  headless: ['@xneog/dsh-base', '@xneog/dsh-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@xneog/dsh-base', '@xneog/dsh-web-app', '@xneog/dsh-headless'],
}

/** The bundle list a `xneog plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@xneog/dsh-base']
 ...
 * `@xneog/dsh-base` (and every other in-box bundle) always comes from
...
export function resolveBundleDir(
  ...
  throw new Error(
    ...
    + `run 'xneog plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}
...
export function loadProfile(
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'xneog plugin --profile ${name} add <package>'`,
      )
    }
