...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@xneog', 'dsh-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@xneog', 'dsh-system-prompt')
...
describe('addHarnessSourceSection', () => {
  ...
  const EXPECTED = `The xneog implementation checkout is at ${SOURCE_ROOT}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend xneog itself.`
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by xneog.')
