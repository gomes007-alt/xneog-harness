 ...
 * @module @xneog/dsh-jobs-local
 */

import { Context } from '@xneog/cordis'
import z from '@xneog/schemastery'
import type { Agent } from '@xneog/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@xneog/dsh-scope'
import type { ScopeLayer } from '@xneog/dsh-scope'
import { deadline, timeoutOf } from '@xneog/dsh-timeout'
import { JobRegistry, JobId } from '@xneog/dsh-jobs'
import type {
 ...
 * `@xneog/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @xneog/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @xneog/dsh-agent)')
    }
