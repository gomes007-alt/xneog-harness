...
import { bindScopeParent, createScope, scopeOf } from '@xneog/dsh-scope'
import type { ScopeKey } from '@xneog/dsh-scope'
import { JobId } from '@xneog/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@xneog/dsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @xneog/dsh-tool-jobs in its composition)')
  })
