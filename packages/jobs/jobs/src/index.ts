 ...
 * process-local registry lives in `@xneog/dsh-jobs-local`.
 * @module @xneog/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@xneog/dsh-jobs is the abstract job registry seam; load an implementation such as @xneog/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
