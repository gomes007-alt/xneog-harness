 ...
 * @module @xneog/dsh-headless/startup
 */
...
function headlessCommand(): Command {
  return new Command()
    .name('xneog-harness --profile headless')
    .description('Answer one task, print the final assistant message, and exit.')
    .helpOption('-h, --help', 'show this help')
    .argument('[task...]', 'the task text; multiple words are joined by spaces')
    .addHelpText('after', `
Examples:
  xneog-harness --profile headless "run the tests"     answer one task and exit
`)
}
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    if (task.trim() === '') program.error('error: a task is required, for example: xneog-harness --profile headless "run the tests"')
