/**
 * @xneog/dsh-web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@xneog/dsh-app-boot'
import * as FrontendStatic from '@xneog/dsh-host-frontend-static'
import type {} from '@xneog/cordis-plugin-loader'
import type {} from '@xneog/dsh-host-webserver'
import type {} from '@xneog/dsh-system-prompt'
import type {} from '@xneog/dsh-shell-env'
...
/** Model-visible orientation and acceptance boundary for sessions created through `xneog-harness web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the xneog Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only xneog-harness web injects window.__DSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@xneog/dsh-web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the xneog Web GUI serving this session.' },
        },
  ...
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`xneog-harness web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
