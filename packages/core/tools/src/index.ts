 ...
 * execution pipeline.
 * @module @xneog/dsh-tools
 */

import { Context, Service } from '@xneog/cordis'
import z from '@xneog/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@xneog/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@xneog/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@xneog/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@xneog/dsh-llm'
import type { Agent } from '@xneog/dsh-agent'
import { snapshotJsonValue } from '@xneog/dsh-session'
import type { JsonValue, UserMessage } from '@xneog/dsh-session'
import type { ToolProviderResult } from '@xneog/dsh-system-prompt'
import type { CodeRuntime } from '@xneog/dsh-code-runtime'
...
export type { JsonValue } from '@xneog/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@xneog/dsh-tools`
...
declare module '@xneog/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@xneog/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@xneog/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@xneog/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@xneog/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@xneog/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @xneog/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
