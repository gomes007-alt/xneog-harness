...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@xneog/dsh-tool-call-timeout-policy` wrapper.
