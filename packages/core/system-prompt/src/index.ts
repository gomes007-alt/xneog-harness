 ...
 *
 * @module @xneog/dsh-system-prompt
 */

import { Context, Service } from '@xneog/cordis'
import z from '@xneog/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@xneog/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@xneog/dsh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@xneog/dsh-llm'
...
declare module '@xneog/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@xneog/dsh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by xneog.',
      })
    }
    this.section({
