...
import LocalCredentialProvider from '@xneog/dsh-credentials-local'
import { settingsNamespace } from '@xneog/dsh-settings'
import FileSettingsProvider from '@xneog/dsh-settings-file'
import { getOrCreateAnonymousUserId } from '@xneog/dsh-anonymous-user-id'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@xneog/dsh-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@xneog/dsh-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@xneog/dsh-settings-file', FileSettingsProvider],
    ['@xneog/dsh-credentials-local', LocalCredentialProvider],
    ['@xneog/dsh-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
