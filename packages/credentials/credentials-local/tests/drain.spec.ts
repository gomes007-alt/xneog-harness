...
vi.mock('@xneog/dsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@xneog/dsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@xneog/dsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
