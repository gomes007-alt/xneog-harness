 ...
 *
 * @module @xneog/dsh-tool-fs-search/search-core
 */
...
import type { RetainedItems } from '@xneog/dsh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@xneog/dsh-subprocess'
 ...
 * `@xneog/dsh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@xneog/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@xneog/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@xneog/dsh-tool-fs-search/presentation} `grepSearchMeta`)
