 ...
 * `@xneog/dsh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@xneog/dsh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@xneog/dsh-tool-fs`),
 * exactly as bash's does.
 *
 * @module @xneog/dsh-fs-sandbox
 */

import { Context } from '@xneog/cordis'
import { LocalFileSystem } from '@xneog/dsh-fs-local'
import type { Config as LocalConfig } from '@xneog/dsh-fs-local'
import { FsError } from '@xneog/dsh-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@xneog/dsh-fs'
import { writableRoots } from '@xneog/dsh-sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@xneog/dsh-sandbox'
import type {} from '@xneog/dsh-sandbox-policy'
