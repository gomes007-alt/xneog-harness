/**
 * Named wire types for the xneog SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@xneog/dsh-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `deepseek-harness-sdk-runtime`.
 *
 * @module @xneog/dsh-sdk-protocol/types
 */

import type { ContentBlock } from '@xneog/dsh-llm'
import type { SessionEvent } from '@xneog/dsh-session'
import type { SubagentStopReason } from '@xneog/dsh-subagent'
