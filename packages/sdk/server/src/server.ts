 ...
 *
 * @module @xneog/dsh-sdk-jsonrpc-server/server
 */

import type { Context } from '@xneog/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@xneog/dsh-agent'
import { createUserMessage } from '@xneog/dsh-llm'
import { carrierKeyOf, type Scoped } from '@xneog/dsh-scope'
import { SessionId } from '@xneog/dsh-session'
import type SubagentRuntime from '@xneog/dsh-subagent'
import type { SubagentRunEndInfo } from '@xneog/dsh-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown xneog SDK runtime method: ${method}`)
    }
  }
  ...
  private async createSession(sessionId: string): Promise<SessionRecord> {
    ...
    // (@xneog/dsh-agent-presets README, "Composing a child agent").
