...
describe('dsh path helpers', () => {
  it('owns the shared default DSH home directory name', () => {
    expect(DSH_HOME_DIR_NAME).toBe('.xneog-harness')
    expect(DEFAULT_DSH_HOME_DISPLAY).toBe('~/.xneog-harness')
    expect(defaultDshHome()).toBe(join(homedir(), '.xneog-harness'))
  })
  ...
  it('labels a resolved home by whether it is the default root', () => {
    expect(dshHomeDisplay(resolve(defaultDshHome()))).toBe('~/.xneog-harness')
