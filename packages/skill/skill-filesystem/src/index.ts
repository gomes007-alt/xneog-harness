 ...
 *
 * @module @xneog/dsh-skill-filesystem
 */
...
import type Schema from '@xneog/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@xneog/dsh-fs'
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.xneog-harness/skills'), source: 'project-dsh', rank: PROJECT_DSH_RANK, projectRoot },
