...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@xneog/dsh-skill`; the durable session catalogs and model-facing loader tool remain in `@xneog/dsh-tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | xneog config root resolved by [`@xneog/dsh-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
