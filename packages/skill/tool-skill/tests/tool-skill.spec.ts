...
import { createUserMessage, CallId, type Message } from '@xneog/dsh-llm'
import { createScope, type Scope } from '@xneog/dsh-scope'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@xneog/dsh-session'
import SystemPrompt, { renderPrompt } from '@xneog/dsh-system-prompt'
import ToolRuntime, { defineContentToolFixture } from '@xneog/dsh-tools'
import AgentRegistry, { agentEvents, Inbox, type Agent, type PreStepDecision } from '@xneog/dsh-agent'
...
describe('dsh-tool-skill', () => {
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.xneog-harness/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.xneog-harness/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.xneog-harness/skills/project-skill')}`,
