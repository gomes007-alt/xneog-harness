...
} from '@xneog/dsh-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@xneog/dsh-tools', 'ToolInput')).toBe('@xneog/dsh-tools#ToolInput')
    expect(typertPackageKey('@xneog/dsh-tools', 'host')).toBe('@xneog/dsh-tools#host')
    expect(ctx.typert.get('@xneog/dsh-tools#ToolInput')).toMatchObject({
      package: '@xneog/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@xneog/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@xneog/dsh-tools', 'host')).toMatchObject({
      key: '@xneog/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@xneog/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@xneog/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@xneog/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@xneog/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@xneog/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@xneog/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@xneog/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@xneog/dsh-tools#ToolInput')).not.toBe(projected)
  })
