...
import { installModelSelection } from '@xneog/dsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@xneog/dsh-agent'
import type {} from '@xneog/dsh-agent-presets/types'
import { AttachmentError } from '@xneog/dsh-attachment'
import type { ImageAttachmentRef } from '@xneog/dsh-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@xneog/dsh-llm'
import { errorChain } from '@xneog/dsh-llm'
import type { ContentBlock, MessageSource } from '@xneog/dsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@xneog/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@xneog/dsh-session'
import type { SessionPersistence } from '@xneog/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@xneog/dsh-session-query'
import { SubagentError } from '@xneog/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@xneog/dsh-subagent'
import { isUserInvocable } from '@xneog/dsh-skill'
...
} from '@xneog/dsh-agent-presets'
import type { PresetBearingSession } from '@xneog/dsh-agent-presets'
import type {} from '@xneog/dsh-tools'
import type {
...
import type { SessionRawArtifact } from '@xneog/dsh-session-persistence'
import {
...
import type {} from '@xneog/dsh-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@xneog/dsh-jobs'
import type { JobSnapshot } from '@xneog/dsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@xneog/dsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@xneog/dsh-goal'
import type { GoalRef as CoreGoalRef } from '@xneog/dsh-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@xneog/dsh-commands'
...
import type {} from '@xneog/dsh-cordis-host-runner/types'
import type {} from '@xneog/dsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@xneog/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@xneog/dsh-settings'
import { credentialRef } from '@xneog/dsh-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@xneog/dsh-session-title'
import type { CallId } from '@xneog/dsh-llm/brand'
import type { ScopeKey } from '@xneog/dsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@xneog/dsh-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@xneog/dsh-user-approval'
...
} from '@xneog/dsh-user-questions'
import { UserQuestionError } from '@xneog/dsh-user-questions'
import { DirectoryPickerError } from '@xneog/dsh-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @xneog/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @xneog/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @xneog/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @xneog/dsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @xneog/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @xneog/dsh-skill', details: {} })
        }
