...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@xneog/dsh-tools'
import { assembleContextFor, type Agent } from '@xneog/dsh-agent'
...
import type { SubagentStartRequest } from '@xneog/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @xneog/dsh-jobs')
  })
